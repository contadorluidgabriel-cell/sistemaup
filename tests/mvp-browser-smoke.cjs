...
(async()=>{
  ...
  assert.equal(plan?.version,3,'prescription v3 was not activated');
  assert.equal(plan?.generator,'system-v3','training plan was not generated by v3');
  ...
  assert.ok(await page.locator('#planEditorBody .pe-session').count()>=1,'editable plan has no sessions');
  await page.click('#cancelPlanEditor');
