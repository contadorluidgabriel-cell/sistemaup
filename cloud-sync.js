(()=>{
  ...
  const TABLES={profiles:'muscle_profiles',state:'muscle_state',workouts:'muscle_workout_records'};
  ...
  function setStatus(label,text){
    ...
    if(text){
      const output=document.getElementById('cloudAccountText');
  ...
  function ensurePanel(){
    ...
    panel.innerHTML=`
      <div class="cloud-account-head">
        <div>
          <span class="screen-label">CONTA E SINCRONIZAÇÃO</span>
          <strong id="cloudAccountTitle">Conecte seu Sistema</strong>
          <p id="cloudAccountText">Seu treino continua local. A conta adiciona backup de perfil, plano, histórico e progressões.</p>
        </div>
        <span class="cloud-state" id="cloudState">OFFLINE</span>
      </div>
      <form class="cloud-auth-form" id="cloudAuthForm">
        ...
        <div class="cloud-auth-actions">
          <button type="submit" class="start" id="cloudLogin">ENTRAR</button>
          <button type="button" class="ghost-action" id="cloudCreate">CRIAR CONTA</button>
        </div>
      </form>
      <div class="cloud-session" id="cloudSession" hidden>
        ...
        <div class="cloud-session-actions">
          <button type="button" class="ghost-action" id="cloudSyncNow">SINCRONIZAR AGORA</button>
          <button type="button" class="cloud-logout" id="cloudLogout">SAIR</button>
        </div>
      </div>
  ...
  function renderSession(){
    ...
    if(user){
      ...
      setStatus(navigator.onLine?'CONECTADO':'OFFLINE',blocked?'Missão local preservada. A sincronização completa continua depois da conclusão.':'Dados locais preservados. A nuvem sincroniza quando houver conexão.');
  ...
  async function pushLocal(){
    ...
    if(profile){
      const {error}=await client.from(TABLES.profiles).upsert({user_id:userId,profile,updated_at:now},{onConflict:'user_id'});
    ...
    if(!stateSyncBlocked()){
      ...
      if(state.plan||state.nextTargets||state.onboarding){
        const {error}=await client.from(TABLES.state).upsert({user_id:userId,state,updated_at:now},{onConflict:'user_id'});
        if(error)throw error;
      }
    }
    ...
    if(Array.isArray(history)&&history.length){
      const rows=history.map((record,index)=>({
        user_id:userId,
        workout_id:workoutId(record,index),
        completed_at:record.completedAt||now,
        payload:record,
        updated_at:now
      }));
      const {error}=await client.from(TABLES.workouts).upsert(rows,{onConflict:'user_id,workout_id'});
      if(error)throw error;
    }
  }
  ...
  function restoreMissingState(remoteState){
    ...
    if(localStorage.getItem(SESSION_KEY)===null&&Number.isFinite(Number(remoteState.sessionIndex))){
      localStorage.setItem(SESSION_KEY,String(Math.max(0,Number(remoteState.sessionIndex))));
      changed=true;
    }
    return changed;
  }
  ...
  async function pullRemote(){
    ...
    if(!localProfile){
      const {data,error}=await client.from(TABLES.profiles).select('profile').eq('user_id',userId).maybeSingle();
      if(error)throw error;
      if(data?.profile){writeJSON(PROFILE_KEY,data.profile);restored=true;}
    }

    if(!stateSyncBlocked()){
      const {data,error}=await client.from(TABLES.state).select('state').eq('user_id',userId).maybeSingle();
      if(error)throw error;
      if(data?.state&&restoreMissingState(data.state))restored=true;
    }

    const {data:remote,error}=await client.from(TABLES.workouts)
      .select('workout_id,payload,completed_at')
      .eq('user_id',userId)
    ...
    if(combined.length){
      writeJSON(HISTORY_KEY,combined);
      if(combined.length!==previousLength)restored=true;
    }
    return restored;
  }
  ...
  async function synchronize({silent=false}={}){
    ...
    try{
      ...
      await pushLocal();
      const meta={lastSync:new Date().toISOString(),userId:session.user.id,stateProtected:stateSyncBlocked(),schemaVersion:2};
  ...
  async function boot(){
    ...
    try{
      ...
      console.error('[Sistema Cloud] Falha ao iniciar',error);
      setStatus('OFFLINE');
    }
  }
  ...
  window.SistemaCloud={sync:synchronize,isConfigured:configured,getSession:()=>session,tables:{...TABLES}};
  boot();
})();
