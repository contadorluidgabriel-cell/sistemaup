(()=>{
  ...
  function ensurePanel(){
    ...
    panel.innerHTML=`
      <div class="cloud-account-head"><div><span class="screen-label">CONTA E SINCRONIZAÇÃO</span><strong id="cloudAccountTitle">Conecte seu Sistema</strong><p id="cloudAccountText">Seu treino continua local. A conta adiciona backup e sincronização entre aparelhos.</p></div><span class="cloud-state" id="cloudState">OFFLINE</span></div>
  ...
  function renderSession(){
    ...
    if(user){
      ...
      setStatus(navigator.onLine?'CONECTADO':'OFFLINE','Dados locais preservados. A nuvem sincroniza quando houver conexão.');
  ...
  async function pushLocal(){
    ...
    if(profile){
      const {error}=await client.from('muscle_profiles').upsert({user_id:userId,profile,updated_at:new Date().toISOString()},{onConflict:'user_id'});
      if(error)throw error;
    }
    const history=readJSON(HISTORY_KEY,[]);
    if(Array.isArray(history)&&history.length){
      const rows=history.map((record,index)=>({user_id:userId,workout_id:workoutId(record,index),completed_at:record.completedAt||new Date().toISOString(),payload:record}));
  ...
  async function pullRemote(){
    ...
    if(combined.length)writeJSON(HISTORY_KEY,combined);
  }
  ...
  async function synchronize({silent=false}={}){
    ...
    try{
      ...
      await pullRemote();
      await pushLocal();
      const meta={lastSync:new Date().toISOString(),userId:session.user.id};
      writeJSON(META_KEY,meta);
      renderSession();
      window.dispatchEvent(new Event('sistema:cloud-synced'));
      if(!silent)toast('Sistema sincronizado com a nuvem.');
    }catch(error){
  ...
  window.SistemaCloud={sync:synchronize,isConfigured:configured};
  boot();
})();
