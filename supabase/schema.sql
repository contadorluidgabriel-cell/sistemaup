...
-- Shared personal Supabase project. All app-owned tables use the muscle_ prefix.
...
create table if not exists public.muscle_workout_records (
...
create index if not exists muscle_workout_records_user_completed_idx
  on public.muscle_workout_records (user_id, completed_at desc);

alter table public.muscle_profiles enable row level security;
alter table public.muscle_state enable row level security;
alter table public.muscle_workout_records enable row level security;
...
drop policy if exists "muscle_workout_records_select_own" on public.muscle_workout_records;
create policy "muscle_workout_records_select_own"
  on public.muscle_workout_records for select
  using (auth.uid() = user_id);

drop policy if exists "muscle_workout_records_insert_own" on public.muscle_workout_records;
create policy "muscle_workout_records_insert_own"
  on public.muscle_workout_records for insert
  with check (auth.uid() = user_id);

drop policy if exists "muscle_workout_records_update_own" on public.muscle_workout_records;
create policy "muscle_workout_records_update_own"
  on public.muscle_workout_records for update
  using (auth.uid() = user_id)
  with check (auth.uid() = user_id);

drop policy if exists "muscle_workout_records_delete_own" on public.muscle_workout_records;
create policy "muscle_workout_records_delete_own"
  on public.muscle_workout_records for delete
  using (auth.uid() = user_id);
